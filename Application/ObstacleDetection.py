...
while (1):
    ...
    edges = cv2.Canny(blur, 100, 200)  # Obtaining clear edges using canny edge detector.
    ...
    if forwardEdge[0] > 350:  # Checking for the object at the front is close to bot.
        if farthest_point[1] < 350:  # Checking for the farthest_point on the left of the frame.
            direction = "Move left "
            print(direction)
        else:
    ...
    if testmode == 1:
        ...
        navigation = cv2.putText(frame, direction, (230, 130), font, 2, (0, 0, 255), 4, cv2.LINE_AA)
